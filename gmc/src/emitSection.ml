...
let match_rule ctors judgs tags builtins (premises, conclusion, name, enum) =
    ...
    let add_qmap m = function
        | Game.QVar i -> QMap.add i (-1) m
        | Game.QVar_btin (cat, i) -> QMap.add i cat m
        | QStr _ -> m
    in
